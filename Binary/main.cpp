...
void add()
{
	...
	ofstream myfile;
     char chr;
       myfile.open("trial.dat",ios::binary|ios::app);
...
void display()// problem doesn't display alone without add first
{
    customers a;
    ifstream myfile;
    myfile.open("trial.dat",ios::in|ios::binary);
...
void search(){// add choice of try again for not found
  ifstream myfile;
  myfile.open("trial.dat",ios::binary);
 ...
 void modify(char *searchkey){
    ...
    bool found = false;
     ...
     fstream myfile;
    myfile.open("trial.dat", ios:: binary|ios::in|ios::out);
    myfile.seekg(0);
    if(!myfile){
        ...
        while(myfile.read(reinterpret_cast<char*>(&a),sizeof(customers))){
    /**/     if(conv(searchkey) == conv(a.first_name)){
...
int main()
{
	...
	customers b/*[50]*/;
	system("color 1f");
	fstream myfile("trial.dat",ios::out|ios::app|ios::binary);
	menu_lable:
